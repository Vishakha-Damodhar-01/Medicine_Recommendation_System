...
# Load model
try:
    gba = pickle.load(open('gba.pkl', 'rb'))
    logger.info("Model loaded successfully")
except Exception as e:
    logger.error(f"Error loading model: {str(e)}")
    raise

# Helper function to fetch disease data
def helper(dis):
    logger.debug(f"Fetching data for disease: {dis}")
    try:
        # Normalize disease name
        dis = dis.strip()
        desc = description[description['Disease'].str.strip() == dis]['Description']
        desc = desc.iloc[0] if not desc.empty else "No description available."

        pre = precautions[precautions['Disease'].str.strip() == dis][['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']]
        pre = [col for col in pre.values.flatten() if pd.notna(col)] if not pre.empty else []

        med = medications[medications['Disease'].str.strip() == dis]['Medication']
        med = json.loads(med.iloc[0].replace("'", '"')) if not med.empty else []

        die = diets[diets['Disease'].str.strip() == dis]['Diet']
        die = json.loads(die.iloc[0].replace("'", '"')) if not die.empty else []

        wrkout = workout[workout['disease'].str.strip() == dis]['workout'].tolist()

        logger.debug(f"Data fetched for {dis}: desc={len(desc)}, precautions={len(pre)}, medications={len(med)}, diets={len(die)}, workouts={len(wrkout)}")
        return desc, pre, med, die, wrkout
    except Exception as e:
        logger.error(f"Error in helper for disease {dis}: {str(e)}")
...
# Diseases list (excluding Common Cold)
diseases_list = {
    ...
    36: 'Tuberculosis', 34: 'Pneumonia', 13: 'Dimorphic hemmorhoids(piles)', 18: 'Heart attack',
...
def get_predicted_value(patient_symptoms):
    logger.debug(f"Processing symptoms: {patient_symptoms}")
    input_vector = np.zeros(len(symptoms_dict))
    for item in patient_symptoms:
        if item in symptoms_dict:
            input_vector[symptoms_dict[item]] = 1
        else:
            logger.warning(f"Invalid symptom: {item}")
            raise ValueError(f"Invalid symptom: {item}")
    predicted_idx = gba.predict([input_vector])[0]
    logger.debug(f"Predicted index: {predicted_idx}")
    if predicted_idx not in diseases_list:
        logger.error(f"Predicted index {predicted_idx} not in diseases_list")
        raise ValueError(f"Predicted disease index {predicted_idx} not recognized")
    predicted_disease = diseases_list[predicted_idx]
